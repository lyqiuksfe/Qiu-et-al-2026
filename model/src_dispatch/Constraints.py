        ...
        for n in range(data.num_nodes):
            for t in range(data.num_rep_hours):
                if Setting['evaluation_type']=='in-sample':
                    ...
                    Model.add_linear_constraint(DV.generation_thermal[g,n,t]-data.Nodes[n].thermal_existing[gtype], poi.Leq, 0)
    ...
    for g in range(data.num_generators_vRE):
        ...
        for n in range(data.num_nodes):
            for t in range(data.num_rep_hours):
                if Setting['evaluation_type']=='in-sample':
                    for u in range(max_gen_per_node):
                        Model.add_linear_constraint(DV.generation_vRE[g,n,u,t]-DV.gen_operational_vRE[g,n,u], poi.Leq, 0)
                else:
                    Model.add_linear_constraint(DV.generation_vRE[g,n,t]-data.Nodes[n].vRE_gen[vRE_type][data.rep_hours[t]], poi.Leq, 0)
        ...
        for n in range(data.num_nodes):
            for t in range(1, data.num_rep_hours):
                if Setting['evaluation_type']=='in-sample':
                    ...
                    Model.add_linear_constraint(DV.generation_thermal[g,n,t]-DV.generation_thermal[g,n,t-1]-data.Generators_thermal[g].ramp_rate*data.Nodes[n].thermal_existing[gtype], poi.Leq, 0)
                    Model.add_linear_constraint(-DV.generation_thermal[g,n,t]+DV.generation_thermal[g,n,t-1]-data.Generators_thermal[g].ramp_rate*data.Nodes[n].thermal_existing[gtype], poi.Leq, 0)
    ...
    for n in range(data.num_nodes):
        for t in range(data.num_rep_hours):
            if Setting['evaluation_type']=='in-sample':
                ...
                Con.load_balance[n,t] = Model.add_linear_constraint(
                    poi.quicksum(DV.generation_vRE[g,n,t] for g in range(data.num_generators_vRE)) +
                    poi.quicksum(DV.generation_thermal[g,n,t] for g in range(data.num_generators_thermal)) -
                    poi.quicksum(data.Nodes[n].arc_signs[l]*DV.flow[data.Nodes[n].arcs[l],t] for l in range(len(data.Nodes[n].arcs))) +       
                    poi.quicksum(DV.storage_discharge[s,n,t] for s in range(data.num_storages)) - 
                    poi.quicksum(DV.storage_charge[s,n,t] for s in range(data.num_storages)) +                                                                              
    ...
    for l in range(data.num_lines):
        for t in range(data.num_rep_hours):
            if Setting['evaluation_type']=='in-sample':
                ...
                Model.add_linear_constraint(DV.flow[l,t]-data.Lines[l].existing_cap, poi.Leq, 0)
                Model.add_linear_constraint(-DV.flow[l,t]-data.Lines[l].existing_cap, poi.Leq, 0)
        ...
        for n in range(data.num_nodes):
            for t in range(data.num_rep_hours):
                if t>0:
                    ...
                    if Setting['evaluation_type']=='in-sample':
                        Model.add_linear_constraint(DV.SOC[s,n,t]- data.Nodes[n].storage_capacity_existing*data.Storages[s].duration_range/2, poi.Eq, 0)
                    else:
                        Model.add_linear_constraint(DV.SOC[s,n,t]- data.Nodes[n].storage_capacity_existing*data.Storages[s].duration_range/2, poi.Eq, 0)
                if Setting['evaluation_type']=='in-sample':
                    ...
                    Model.add_linear_constraint(data.Nodes[n].storage_capacity_existing-DV.storage_charge[s,n,t], poi.Geq, 0)
                    Model.add_linear_constraint(data.Nodes[n].storage_capacity_existing-DV.storage_discharge[s,n,t], poi.Geq, 0)
                    Model.add_linear_constraint(data.Nodes[n].storage_capacity_existing*data.Storages[s].duration_range-DV.SOC[s,n,t], poi.Geq, 0)

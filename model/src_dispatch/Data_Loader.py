...
class Node:
    ...
    def populate_node_data(self, Setting, Nodes):
        ...
        if Setting['evaluation_type']=='in-sample':
            sce = Setting["inv_sce"]
        elif Setting['evaluation_type']=='out-of-sample':
        ...
        for g in range(len(self.Generators_vRE)):
            ...
            if Setting['evaluation_type']=='in-sample':
                ...
                nodal_capacity_list=pd.read_csv(f'{Setting["input_prefix"]}_nodal_capacity.csv')
        for n in range(len(df_nodes)):
            ...
            for g in range(len(self.Generators_vRE)):
                ...
                if Setting['evaluation_type']=='in-sample':
                    ...
                    en.vRE_existing[vRE_type] = vRE_capacity_list[vRE_type].iloc[start:stop].values
                    gen = vRE_cf_df_all[vRE_type].iloc[:, start:stop].values*en.vRE_existing[vRE_type]
                    en.vRE_gen[vRE_type]=gen.sum(axis=1)
            
            for g in range(len(self.Generators_thermal)):
                thermal_type=self.Generators_thermal[g].Type
                if Setting['evaluation_type']=='in-sample':
                    if not Setting['greenfield_thermal']:
                        en.thermal_existing[thermal_type] = nodal_capacity_list[thermal_type].values[n]
                    else:
                        en.thermal_existing[thermal_type] = 0.0
                else:
                    en.thermal_existing[thermal_type] = nodal_capacity_list[f'Total_{thermal_type}'].values[n]
            ...
            if Setting['evaluation_type']=='in-sample':
                ...
                en.storage_capacity_existing=nodal_capacity_list['Total_storage_capacity'].values[n]
            Nodes.append(en)

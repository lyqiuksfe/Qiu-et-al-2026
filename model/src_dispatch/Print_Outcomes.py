...
def publish_extensive_form(DV_values, Duals, data, Setting):
    ...
    for g in range(data.num_generators_vRE):
        if Setting['evaluation_type']=='out-of-sample':
            flow_output[f'{data.Generators_vRE[g].Type}_load'] = DV_values.generation_vRE[g,:,:].sum(axis=(0))
            flow_output[f'{data.Generators_vRE[g].Type}_prod']=0.0
            for t in range(data.num_rep_hours):
                for g in range(data.num_generators_vRE):
                    gtype=data.Generators_vRE[g].Type
                    flow_output.loc[t,f'{data.Generators_vRE[g].Type}_prod'] = sum(data.Nodes[n].vRE_gen[gtype][data.rep_hours[t]] for n in range(data.num_nodes))
        else:
            flow_output[f'{data.Generators_vRE[g].Type}'] = DV_values.generation_vRE[g,:,:,:].sum(axis=(0,1))

    for g in range(data.num_generators_thermal):
        if Setting['evaluation_type']=='out-of-sample':
            flow_output[f'{data.Generators_thermal[g].Type}_load'] = DV_values.generation_thermal[g,:,:].sum(axis=(0))
        else:
    ...
    if Setting['evaluation_type']=='out-of-sample':
        flow_output.to_csv(f"{Setting['output_fix']}_test_{Setting['test_year']}_dispatch_Load.csv",index=True)
        print(f'saved to {Setting['output_fix']}_test_{Setting['test_year']}_dispatch_Load.csv')
    else:
...
def publish_summary(DV_values, data, Setting, start_time, relative_gap):
    ...
    if Setting['evaluation_type']=='out-of-sample':
        res['test_sce']=Setting['test_sce']
        res['test_year'] = Setting['test_year']
        res['out_of_sample_year_list'] = Setting['out_of_sample_year_list']
    else:
    ...
    for g in range(data.num_generators_vRE):
        ...
        if Setting['evaluation_type']=='in-sample':
            ...
            res[f'{vRE_type}_total_cap']  = sum(data.Nodes[n].vRE_existing[vRE_type] for n in range(data.num_nodes))
        if Setting['evaluation_type']=='in-sample':
            res[f'{vRE_type}_FOM_cost'] = data.Generators_vRE[g].FOM_per_unit * res[f'{data.Generators_vRE[g].Type}_total_cap']
    ...
    for g in range(data.num_generators_thermal):
        ...
        res[f'{data.Generators_thermal[g].Type}_total_cap']  = sum(data.Nodes[n].thermal_existing[gtype] for n in range(data.num_nodes))
        if Setting['evaluation_type']=='in-sample':
            if Setting['thermal_expansion_allowed']:
    ...
    if Setting['evaluation_type']=='in-sample':
        ...
        res['Line_total_cap'] = sum(data.Lines[l].existing_cap for l in range(data.num_lines))
    ...
    if Setting['evaluation_type']=='in-sample':
        ...
        res['storage_total_cap'] = sum(data.Nodes[n].storage_capacity_existing for n in range(data.num_nodes))
    ...
    if Setting['csv_output_file'] is None:
        if Setting['evaluation_type']=='in-sample':
            ...
            csvfile = os.getcwd()+f'/{Setting['balancing_authority']}_dispatch_summary_{Setting['test_sce']}.csv'
    else:

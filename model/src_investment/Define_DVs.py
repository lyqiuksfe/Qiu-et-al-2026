...
def define_decision_variables(Model, DV, data, Setting):
    ...
    DV.generation_vRE = Model.add_variables(range(data.num_generators_vRE), range(data.num_nodes), range(max_gen_per_node), range(data.num_rep_hours), lb=0, domain=poi.VariableDomain.Continuous)
